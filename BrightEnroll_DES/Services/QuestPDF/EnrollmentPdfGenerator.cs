...
public class EnrollmentPdfGenerator
{
    ...
    public async Task<byte[]> GenerateEnrollmentReportAsync(string reportType)
    {
        ...
        switch (reportType.ToLower())
        {
            ...
            case "enrolled":
                ...
                return await GenerateEnrolledReportAsync(stats);
            default:
                        ...
                        headerColumn.Item().PaddingTop(8).Row(detailsRow =>
                        {
                            ...
                            detailsRow.RelativeItem().AlignRight().Column(rightDetailsCol =>
                            {
                                rightDetailsCol.Item().Row(row =>
                                {
                                    ...
                                    rightDetailsCol.Item().Text("System").FontSize(9).FontColor(global::QuestPDF.Helpers.Colors.Black); // Enrollment reports are system-generated
                                });
                            });
                        });
                    });
    ...
    private async Task<byte[]> GenerateEnrolledReportAsync(EnrollmentStatistics stats)
    {
                        ...
                        headerColumn.Item().PaddingTop(8).Row(detailsRow =>
                        {
                            ...
                            detailsRow.RelativeItem().AlignRight().Column(rightDetailsCol =>
                            {
                                rightDetailsCol.Item().Row(row =>
                                {
                                    ...
                                    rightDetailsCol.Item().Text("System").FontSize(9).Bold();
                                });
                            });
                        });
                    });
